        ...
        while(posts.length > 0) {
            ...
            const results = await Promise.allSettled(
                batch.map(async (post) => {
                    const { postNum } = post;
                    const url = URL_PATTERNS.POST_MOB(this.galleryId, postNum);
                    try {
                        const response = await this.fetchUtil.axiosFetcher(url, 'GET', {...headers_mob_chrome_gallog}, 1, null, 20000, 2000);
                        if(!limit) limit = response.headers['x-ratelimit-limit'];
                        remaining = Math.min(remaining, response.headers['x-ratelimit-remaining'] || 0);
        ...
        while(UIDs.length > 0) {
            ...
            const results = await Promise.allSettled(
                batch.map(async (uid) => {
                    ...
                    try {
                        const response = await this.fetchUtil.axiosFetcher(url, 'GET', {...headers_mob_chrome_gallog}, 1, null, 20000, 2000);
                        if(!limit) limit = response.headers['x-ratelimit-limit'];
                        remaining = Math.max(remaining, response.headers['x-ratelimit-remaining'] || 0);
            ...
            results.forEach(result => {
                ...
                if(status === "fulfilled"){
                    ...
                    if(response.status === 403) { // mob: 403 === 삭제된 갤로그, des: 404 === 삭제된 갤로그
